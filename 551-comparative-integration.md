...
|--------|--------------|-------|----------------|-------|--------|----------|
| **Commerce** | ★★★½ (3.5/5) | ✅✅ 154 LOC | ✅✅ Complete | ⚠️ Stub | ❌ None | ✅✅ TLS/mTLS |
| **Sales** | ★★½ (2.5/5) | ✅ 62 LOC | ✅ Business logic | ⚠️ RED | ✅ CRD | ⚠️ Insecure |
| **Transformation** | ★★½ (2.5/5) | ❌ None | ✅ MCP arch | ❌ Stub | ❌ None | ✅ Token |
...
- ❌ No integration test implementation
...
**Strengths:**
- **Well-structured EDA schema**: 62 LOC `integration_facts.proto`
...
**Gaps:**
- ⚠️ Tests in RED state (placeholder at `internal/tests/execute_test.go`, 20 LOC)
...
- **Centralized security**: Uses `ameide_mcp` package for transport/security
...
- **Clean separation**: `internal/mcp/` (protocol), `internal/adapter/` (business logic), factory via `amcp.NewHTTPMux()`

**Gaps:**
- ❌ No domain-specific proto definitions
- ❌ No business logic (placeholder structure only)
- ❌ Tests stub-only (6 LOC empty smoke test at `tests/smoke_test.go`)
...
**Strengths:**
- **⭐ Comprehensive tests**: 245+ LOC across 5 files (`auth_test.go`, `http_test.go`, `origin_test.go`, `oauth_test.go`)
...
| Sales | 1 | 62 | EDA facts (3 types) | Generic integration | ⭐⭐⭐⭐ |
| Transformation | 0 | 0 | None | None | ⭐ |
| SRE | 0 | 0 | None | Generic only | ⭐ |

### Test Coverage Maturity

| Domain | Files | LOC | Coverage | Status |
|--------|-------|-----|----------|--------|
| SRE (MCP) | 5 | 245+ | Security, protocol, OAuth | ✅ PASS ⭐⭐⭐ |
| Sales | 1 | 20 | Handler echo | ⚠️ RED |
...
- Sales: 20 LOC RED placeholder
- **No integration tests actually implemented** in any domain
...
- **Sales**: `packages/ameide_core_proto/src/ameide_core_proto/sales/integration/v1/`
  - `integration_facts.proto` (62 LOC)
- **Transformation**: None
- **SRE**: None (uses generic `integration/v1` only)
