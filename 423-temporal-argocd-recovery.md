...
1) **Schema init/update issues → CrashLoop**
   - Temporal services can crash during the SQL schema compatibility check (e.g., `schema_version` missing or partially applied).
   - In the operator-based model, schema setup/update is performed by operator-owned Jobs created from the `TemporalCluster` reconcile.

2) **Cluster metadata drift → panic**
   - Symptom: `panic: Cluster info initial versions have duplicates`.
   - Common cause during migrations: the Temporal DB contains multiple cluster entries (e.g., both `active` and `temporal`) in `cluster_metadata_info`.

3) **Namespace metadata missing → startup error**
   - Symptom: `Failed to lock namespace metadata. Error: sql: no rows in result set`.
   - Root cause: missing `namespace_metadata` row for the Temporal metadata partition (Postgres `partition_id=54321`).

## Fixes applied (generic recipe)

Recovery is operator-first: make sure the CRDs + controller are healthy, then resync `{env}-data-temporal` and debug via `TemporalCluster` status conditions + operator logs.

1) **Ensure operator apps are Healthy**
   - ArgoCD apps: `cluster-crds-temporal-operator`, `cluster-temporal-operator`
   - `kubectl -n argocd logs deploy/temporal-operator-controller-manager` (or equivalent) for controller errors.

2) **Resync the environment Temporal app**
   - ArgoCD app: `{env}-data-temporal` (deploys `TemporalCluster` + `TemporalNamespace` CRs in the environment namespace).

3) **Inspect `TemporalCluster` reconcile and schema Jobs**
   - `kubectl -n <env-namespace> get temporalcluster temporal -o yaml`
   - Look for operator-created setup/update pods/jobs (names typically include `temporal-setup-*` / `temporal-update-*`) and inspect their logs.
   - If you need to force a rerun, delete the failed/completed schema pod/job; the operator will recreate it on the next reconcile.

4) **If you see `initial versions have duplicates`**
   - Check `cluster_metadata_info` and remove the stale cluster name row (example shown for a leftover `active` cluster):
     - `DELETE FROM cluster_metadata_info WHERE cluster_name='active';`

5) **If you see `Failed to lock namespace metadata`**
   - Ensure the metadata partition row exists:
     - `INSERT INTO namespace_metadata(partition_id, notification_version) VALUES (54321, 0) ON CONFLICT (partition_id) DO NOTHING;`

## Verification steps (dev)

- `argocd app get argocd/{env}-data-temporal` → Health: Healthy; `TemporalCluster` reconciled.
- `kubectl -n <env-namespace> get temporalcluster temporal` → `Ready=True`.
- Temporal pods in `<env-namespace>` all Running; no CrashLoopBackOff.
- (Optional) From `temporalio/admin-tools` in the namespace: `temporal operator cluster health --address temporal-frontend:7233`.
