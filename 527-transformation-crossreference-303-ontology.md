...
| “Pins” as a typed object (`ChangePins`, `*_ref` fields) | Pins as named links | The simplest handshake uses well-known relationship kinds from the change element, e.g. `ref:requirement`, `ref:deliverables_root`, optional `ref:baseline`, `ref:release`, `evidence:*`. | `backlog/527-transformation-methodology-dictionary.md` | Typed pin payloads can exist later, but named links are the default/simple posture. |
