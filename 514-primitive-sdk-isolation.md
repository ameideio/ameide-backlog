...
**Status:** Partially implemented, strongest for Go/Domain primitives.

- **Go (Domain/Process/Agent/UISurface)**
...
- **TypeScript & Python**
  ...
  - There is no TS/Python import policy in `primitive verify` yet (no automated scan for `@ameide/core-proto` or Python proto imports).
...
- **Module layout**
  - Scaffolders create one module/project per primitive:
    - Go: `go.mod` per Domain/Process primitive.
    - Python: `pyproject.toml` per Agent primitive.
    - TS: `package.json` per UISurface primitive.
...
- **Other languages**
  - Process/Agent/UISurface runtime code is designed to be self-contained and SDK-based by convention, but:
    - TS/Python scaffolds have not yet gained an equivalent automatic “no cross-primitive import” check.
    - There is no central allowlist enforcement for shared libraries beyond the Go import policy.
...
- `primitive scaffold`:
  - Enforces canonical language per kind:
    - Domain/Process: defaults to Go, other languages not wired.
    - Agent: uses the dedicated Python scaffold path.
    - UISurface: uses the TypeScript scaffold path.
