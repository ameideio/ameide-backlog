...
* Dual mode: Helpers enforce `INTEGRATION_MODE` (repo|local|cluster). Some suites only run in cluster (`services/agents_runtime/__tests__/integration` exits in non-cluster). Mock-vs-live coverage is uneven across services.
...
* No single README/contract for env vars (`TEST_ENV`/`TEST_PROFILE` vs `INTEGRATION_MODE`), Tilt buttons, or how to run by service.
