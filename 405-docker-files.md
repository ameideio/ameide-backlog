...
- Earlier drafts used `Dockerfile.release` to mean “production/publishable”. This repo standardizes on `Dockerfile.main` for that role.
...
- **Ring 2 (`Dockerfile.main`)** – staging/prod images also copy workspace SDK packages (no Ameide registry downloads), use strict frozen installs for third-party deps; Ameide packages resolve via `workspace:*`/vendored SDK. SDK publish/smoke is an outer loop.
...
## CI / Prod Dockerfiles (`Dockerfile.main`)
...
- Dockerfile.main COPY and CMD must use nested paths:
  ```dockerfile
...
- `scripts/policy/check_dockerfile_parity.sh` enforces consistency between `Dockerfile.dev` and `Dockerfile.main`:
...
- `scripts/ci/go_use_workspace_sdk.sh` provides a shared Go workspace-SDK hydration step (prime SDK module, disable GOPROXY, apply replace); dev and main Dockerfiles should use it for Go services.
