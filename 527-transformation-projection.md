...
> **Superseded (v6):** repository hierarchy is a **TOGAF 10 Architecture Repository hierarchy** that is **purely projection-derived** (no canonical “workspace tree”).  
> See `backlog/527-transformation-projection-v6.md` and `backlog/701-repository-ui-enterprise-repository-v6.md`.
...
- Clean-target refactor (memory-first, EDA-correct): `backlog/657-transformation-domain-clean-target-v2.md`
...
- [x] Ingestion runner exists (bridge mode; pre-496v2 posture):
  ...
  - [ ] **Clean target (496 v6, 657 v2):** replace bridge mode with event-plane delivery (CloudEvents) as the normative runtime posture, and treat the relay as local/debug/recovery only.
...
- Confirm the operational posture for environments:
  ...
  - the clean target (496 v6, 657 v2) is event-plane delivery; relay is not a default runtime dependency
