...
- Coder CLI auth is treated as a **Vault KV → ESO** secret (`Secret/coder-cli-auth` per workspace namespace), with an in-cluster reconciler that maintains `coder-cli-session-token` in Vault KV using a long-lived `coder-bootstrap-token` seeded in AKV.
