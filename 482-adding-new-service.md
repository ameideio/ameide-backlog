...
| **Testing (unit/integration/e2e)** | Are tests classified and runnable under `ameide dev inner-loop-test` (Phase 0/1/2: contract → unit → integration) and `ameide ci e2e` (cluster-only E2E), and do they emit JUnit evidence? | `backlog/430-unified-test-infrastructure-v2-target.md`, `backlog/537-primitive-testing-discipline.md`, `backlog/468-testing-front-door.md` |
...
6. **Testing**
   - Implement tests under the 430v2 contract (Unit → Integration → E2E) and ensure they run under the repo front doors:
     ...
     - Local agent/human inner loop: `ameide dev inner-loop-test` (Phase 0/1/2 only; local-only).
     - Cluster-only E2E: `ameide ci e2e` (Playwright).
