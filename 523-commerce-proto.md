...
- `496-eda-principles-v6.md` (integration invariants; Kubernetes standard)
- `509-proto-naming-conventions-v6.md` (package/semantic identity conventions)
...
**Kubernetes standard note:** for inter-primitive messaging in Kubernetes, CloudEvents is the canonical envelope and Protobuf payloads MUST NOT embed message metadata as the primary envelope (see `backlog/496-eda-principles-v6.md`). The `CommerceMessageMeta` shown below should be treated as **legacy/migration scaffolding** for older topic-family contracts.

Every intent/fact MUST carry:

- tenant isolation: `tenant_id` (required)
- traceability: CloudEvents `id` (idempotency key) plus correlation/causation + tracing extensions (see `backlog/496-eda-principles-v6.md`)
