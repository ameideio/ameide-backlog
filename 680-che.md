...
- Human workspace UX + guardrails: `backlog/652-agentic-coding-dev-workspace-coder.md`
- Testing layers: `backlog/653-agentic-coding-test-automation-coder.md`
- CLI front doors + evidence layout: `backlog/654-agentic-coding-cli-surface-coder.md`
...
- Running platform verification front doors inside Che environments (`ameide test`, `ameide test cluster`) and emitting evidence in the standard layout.
...
Process/orchestration may cause Git changes (e.g., PRs that update deployable artifacts or GitOps inputs). **Argo CD** remains the mechanism that reconciles deployed state back to Git and provides the “deployed truth” signal used by Phase 5 Playwright E2E (`ameide test cluster`) per `backlog/653-agentic-coding-test-automation-coder.md`.
...
- `ameide test cluster` (Phase 4/5; deployed truth; cluster integration + Playwright against target URL)
...
If a Che task is allowed to interact with cluster resources (example: reading the environment config/secret inputs for `ameide test cluster`, or managing Gateway API routing primitives for an in-cluster test harness), the platform must follow the same vendor-aligned pattern as the Coder provider:
...
- Increase `CheCluster.spec.devEnvironments.startTimeoutSeconds` (e.g. `600–900`) so Che reconciles `DevWorkspaceOperatorConfig/devworkspace-config` (`namespace: eclipse-che`) with a larger `config.workspace.progressTimeout`, and ensure workspace templates use the expected nodeSelector/tolerations for `ameide.io/pool=workspaces-che` (see `backlog/684-aks-node-pool-strategy-option-b.md`).
...
Evidence layout must match `backlog/654-agentic-coding-cli-surface-coder.md`.
