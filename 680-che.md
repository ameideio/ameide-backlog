...
- Increase `DevWorkspaceOperatorConfig.config.workspace.progressTimeout` (e.g. `10–15m`) for the Che deployment, and ensure workspace templates use the expected nodeSelector/tolerations for `ameide.io/pool=workspaces-che` (see `backlog/684-aks-node-pool-strategy-option-b.md`).
