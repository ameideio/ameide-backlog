...
* A short inner loop where one command (`ameide test e2e --inner-loop`) can:
...
## 5) CLI contract (`ameide test e2e --inner-loop`)

One-command flow (agent-friendly, no human intervention):

1. Discover the target environment namespace by locating `Deployment/www-ameide-platform`.
2. Read `AUTH_URL` from `ConfigMap/www-ameide-platform-config` and validate it matches the platform `HTTPRoute` hostname (origin correctness).
3. Derive the platform `parentRefs` (Gateway name/namespace/section) from the existing platform `HTTPRoute` in the environment namespace.
4. Generate:
...
6. Create per-run `Service` + `HTTPRoute` (TTL-labeled) in the workspace namespace.
7. Wait for `HTTPRoute` readiness.
8. Proof-of-routing preflight: fetch `GET /__ameide_innerloop__/probe` through the real origin with both headers and validate run-id.
9. Run Playwright against the real origin while injecting the two headers only for requests to the platform host.
10. Cleanup ALWAYS deletes the route + service.
