...
- **Envoy Gateway topology is standardized (cluster-shared control plane)**: controller runs in `argocd`; per-environment `EnvoyProxy` + `Gateway` resources live in `ameide-{env}`; internal/xDS TLS is owned by Envoy Gateway `certgen` in `argocd`, while external hostname TLS remains cert-manager-managed in the environment namespace.
  - **Follow-up hardening (vendor-aligned)**: express per-environment proxy infra knobs via `Gateway.spec.infrastructure.parametersRef` (namespaced), so environment apps do not need to create/own cluster-scoped `GatewayClass` objects.
