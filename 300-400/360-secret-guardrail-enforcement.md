...
- **2025-11-07 – Helm template CI scaffold**: Added `gitops/ameide-gitops/scripts/validate-hardened-charts.sh`, which renders the hardened charts (agents*, db-migrations, inference-gateway, threads, transformation, workflows) for the local/staging overlays. Wire this script into CI next so regressions surface automatically.
...
- **2026-03-09 – CI enforcement**: The Helm lint workflow (`.github/workflows/ci-helm.yml`) now invokes `gitops/ameide-gitops/scripts/validate-hardened-charts.sh`, so PRs fail immediately when the hardened charts violate `required(...)` guardrails or reference missing ExternalSecrets.
