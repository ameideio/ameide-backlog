...
- **Regression coverage.** Mirror the existing Graph → transformation integration helpers (`services/graph/__tests__/integration/helpers.ts`) for the Scrum tables, and add Jest/Vitest suites for the portal components plus Connect integration tests so `/api/transformations` flows remain green. Historical CI jobs used `services/transformation/__tests__/integration/run_integration_tests.sh`; under 430v2 the contract is `ameide dev inner-loop-test` (Phase 2 = local integration with mocks/stubs) plus `ameide ci e2e` (cluster-only Playwright E2E).
