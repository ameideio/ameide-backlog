...
- **Schema & migrations**
  ...
  - Seed or reference the repository that will hold `feature_request` elements. If we keep them in a dedicated graph, create that repository/node hierarchy via a GitOps seed Job (or via `GraphService` APIs) so the worker can safely target it (Flyway is schema-only).
...
- **Target repository + routing for feature requests is unclear.** Historically `db/flyway/sql/graph/V1__initial_schema.sql` seeded architecture/governance repositories. With Flyway schema-only, decide whether feature requests belong in an explicit GitOps-seeded repository, a new repository, or in tenant-scoped graphs so the worker knows where to create elements.
