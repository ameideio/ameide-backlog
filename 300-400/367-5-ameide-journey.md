...
- **Session + role context.** backlog/329’s requirements are satisfied by the middleware-first session pattern (`backlog/431-auth-session-v2.md`) plus issuer-first routing/access states (`backlog/597-login-onboarding-primitives.md`). Always call `requireOrganizationMember`/`requireOrganizationRole` (`lib/auth/middleware/authorize.ts`) before allowing edits to backlog state so Scrum/SAFe roles stay enforced.  
