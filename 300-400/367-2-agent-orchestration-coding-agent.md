...
- Guardrails need to stay in lockstep with backlog/362. Use `scripts/vault/ensure-local-secrets.py` to register every new CLI/preview secret and `gitops/ameide-gitops/scripts/validate-hardened-charts.sh` to prove the Helm overlays for `agents`, `agents-runtime`, `workflows`, and shared db-migrations fail fast when secrets/ExternalSecrets drift. This keeps Layer 15 as the single source of truth for bootstrap credentials even when repo adapters add new env vars.
