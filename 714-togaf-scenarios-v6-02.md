...
* **As a human (architect)**, I can create a requirement (`REQ-TRAVEL-001`) and a standard that references it, publish both through governed changes, and then open `REQ-TRAVEL-001` and see:
...
* **As an agent**, I can answer “what depends on REQ-TRAVEL-001?” and produce a citeable impact summary by calling Projection queries and reading cited snippets; I cannot create relationships in a DB (because there is no canonical relationship store), and I cannot write without Domain.
...
- **Architect agent deliverables**
  - Can answer “what depends on REQ-TRAVEL-001?” by calling derived backlink/impact queries and returning origin-cited results (“why linked?”).
  - Can produce a citeable context bundle for `REQ-TRAVEL-001` (and its immediate neighbors) that is reproducible at the resolved `read_context`.
...
2. Publish two artifacts via **Domain** governed write loop:
   ...
   * `requirements/REQ-TRAVEL-001.md` as a typed item (YAML frontmatter required):

     * `id: REQ-TRAVEL-001`
     * `scheme: togaf`
     * `type: togaf.requirement`
   * `architecture/standards/standard-a.md` as a typed item (YAML frontmatter required) that references `REQ-TRAVEL-001`:

     * preferred: frontmatter `links:` entries with `target_id: REQ-TRAVEL-001`
     * compatibility: inline `ref: REQ-TRAVEL-001` tokens in body/content (still derived into the same backlinks model)
   * (Minimal “travelling” hop beyond standards) `business/capabilities/CAP-001.md` as a typed item that references `REQ-TRAVEL-001` (same link semantics; used only to prove cross-folder backlinks are deterministic and citeable).
3. Projection resolves `published → resolved.commit_sha` and indexes the published baseline.
4. Projection answers:

   * `GetElementById("REQ-TRAVEL-001")` returns a single path + citation (or deterministic “duplicate id” error if applicable).
   * `ListBacklinks("REQ-TRAVEL-001")` returns at least two backlinks (standard-a + CAP-001) and every edge includes an **origin citation** (where the `ref:` token lived). 
...
Increment 2 makes the travelling requirement “dependable by derivation”:

* The stable id is `REQ-TRAVEL-001`.
* At least one additional artifact outside `requirements/**` and `architecture/standards/**` references the requirement (e.g., `CAP-001`) so cross-folder backlinks are exercised early.
...
* **Duplicate ID:** two files claim `id: REQ-TRAVEL-001` at the same SHA → `GetElementById` fails with explicit “duplicate id” error and includes candidate citations; UI does not pick a winner. 
...
2. Domain enforces minimum authoring policy for participating EA artifacts:
   ...
   * YAML frontmatter required for eligible “EA item” files
   * stable `id` + `scheme` + `type` required for eligible EA items (mandatory in this increment, because deterministic graph semantics require stable identity + type)
...
* Extend publish workflow (from Increment 1) with an artifact validation step:
  ...
  * On `CreateCommit` or `PublishChange` (choose one consistently), validate:
    ...
    * if path is under `requirements/**` then YAML frontmatter must parse and must include `id`, `scheme`, `type`
    * if path is under `architecture/standards/**` then YAML frontmatter must parse and must include `id`, `scheme`, `type`
    * relationship authoring is allowed in two modes:

      * inline links/refs (`links:` frontmatter and/or `ref:` body tokens)
      * relationship-as-item files (required for ArchiMate/BPMN relationship types in later increments, allowed here as a compatible authoring form)
...
* **Unit:** publish rejects `requirements/REQ-TRAVEL-001.md` if stable ID missing (explicit error, deterministic).
* **Unit:** publish accepts `requirements/REQ-TRAVEL-001.md` when ID present and well-formed.
...
* **Stable identity extraction**
  ...
  * Parse YAML frontmatter fields `{id, scheme, type}` for typed items.
  * Index: `item_id → {scheme, type, path, citation}` (citation anchored to `{repo_id, sha, path[, anchor]}`)

* **Inline reference extraction (MVP grammar)**

  * Preferred: parse frontmatter `links:` entries that may reference:

    * `target_id: <ID>` (stable, rename-safe)
    * `target_path: <path>` (Git-path locator, agent-friendly; rename-sensitive)
  * Compatibility: parse inline `ref: <ID>` tokens in body/content (example: `ref: REQ-TRAVEL-001`)
...
* **Backlinks index**
  ...
  * `target_id → [ {source_path, origin_citation, ref_kind} ]` where ref_kind distinguishes:

    * `link.target_id` (frontmatter link)
    * `link.target_path` (frontmatter path link, joined to `target_id` via `id → path` at the same SHA when possible)
...
* **Unit:** `GetElementById("REQ-TRAVEL-001")` returns path + citation at published SHA.
* **Unit:** duplicate ID at same SHA → explicit “duplicate id” error with candidate citations (no arbitrary winner). 
* **Unit:** `ListBacklinks("REQ-TRAVEL-001")` returns `standard-a.md` and `CAP-001.md` and includes origin citations (anchor points to the ref token).
...
GitLab can govern the merge, but Increment 2 requires a platform-owned workflow that is:

* cross-channel (UISurface + MCP) and resumable (restart-safe),
* able to wait for **Projection convergence** (“INDEXED at `target_head_sha`”) as part of the definition of done,
* able to attach/track agent-produced evidence (citeable summaries) without turning GitLab approvals into the platform’s only governance mechanism.
...
1. Agent can answer impact questions (“what depends on REQ-TRAVEL-001?”) by:
...
2. UI supports “open by element ID” (REQ-TRAVEL-001), not path identity.
3. UI provides:
...
* ✅ No relationship CRUD exists in a canonical DB: relationships are authored in Git either as inline links/refs (TOGAF/Markdown) or as first-class relationship items (ArchiMate/BPMN), and Projection derives backlinks/impact from Git at the resolved SHA. 
