...
repo/
  ...
  primitives/
    agent/
      research-agent/               # Human-owned runtime package (never cleaned)
        __init__.py
        app.py                      # tiny entrypoint; imports SDK stubs + generated wiring
        nodes_impl.py               # node implementations (human-owned)
        policy_impl.py              # routing/tool policy (human-owned)
...
* Adds nodes by importing **human-owned** implementations.
...
* Human-owned files live outside generator outputs (e.g., `primitives/agent/<agent-id>/{nodes_impl.py,policy_impl.py,...}`) and are never touched by generators.
...
from acme_agent.nodes_impl import entry  # human-owned
from acme_agent.policy_impl import register_edges  # human-owned
...
* `nodes_impl.py` and `policy_impl.py` (the only human-owned imports)
