...
* **Implementation-owned code surface is tiny:** `*_impl.go` + `*_test.go`. Everything else is regenerated.
...
primitives/process/
  order-fulfillment/                   # IMPLEMENTATION-OWNED (never cleaned)
    internal/
      workflows/
...
### Generated vs implementation-owned: strict contract
...
**Implementation-owned (never overwritten):**
...
* Anything not matching that pattern is treated as implementation-owned and never touched.
