...
1. **Secrets live only in Key Vault (KV)** and are delivered to Kubernetes via **ExternalSecrets**.
2. **Settings live only in GitOps values → rendered ConfigMaps** (ArgoCD-managed).
3. **Everything is deployed only via CI workflows** (no “manual deploy”, no ad-hoc patching for Argo-managed apps).
...
2. **Public API Gateway URL** (browser-visible API origin)
   ...
   - Output env: `NEXT_PUBLIC_ENVOY_URL`, `NEXT_PUBLIC_WS_URL`
...
- **`AUTH_*`**:
  - `AUTH_URL` / `NEXTAUTH_URL` are settings (ConfigMap).
...
- Document and enforce that:
  ...
  - ArgoCD apps are only changed via GitOps PRs.
