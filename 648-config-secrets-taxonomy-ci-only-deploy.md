...
- `AMEIDE_PLATFORM_BASE_URL`
  ...
  - **Where set today**:
    - `ameide test cluster` reads `AUTH_URL` from `ConfigMap/www-ameide-platform-config` and passes it to Playwright as `AMEIDE_PLATFORM_BASE_URL` (and verifies it matches the HTTPRoute host).
