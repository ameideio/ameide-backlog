...
- **A2A REST binding + Agent Card**: defined in 505-v2.  
...
| **Support – CLI & guardrails** | Tooling agents use internally | `484a-ameide-cli-primitive-workflows.md` … `484f` | CLI workflows, repo/gitops handling, prompts |
...
Each hand-off is asynchronous (EDA intents/facts or A2A) and enforced by the operators defined in 495/498/499/500/501. At **runtime**, Process workflows interact with Transformation only via bus messages (`scrum.domain.intents.v1` / `scrum.domain.facts.v1`); any synchronous definition fetch by the Process operator (499) is a **control‑plane** concern, not a domain state transition.
...
**Feeds:** AmeidePO subscribes primarily to process facts (e.g., `SprintBacklogReadyForExecution`, `SprintBacklogItemReadyForWork`), AmeideSA receives repo delegation context through these events/commands, and AmeideCoder receives tasks via A2A.
...
- `500-agent-operator.md` and `504-agent-vertical-slice.md` cover runtime management and CLI guardrails that AmeideCoder uses internally.
- CLI guardrails and prompts are further detailed in the `484a-484f` backlog series; AmeideCoder treats these CLI commands as internal tools per 504.
