...
Update (2026-01): when a workspace needs additional, tightly-scoped permissions (e.g., cluster verification routing primitives or env-namespace reads for `ameide test cluster`), prefer predeclared roles plus bind-only delegation rather than allowing workspace provisioning to author arbitrary Roles.
