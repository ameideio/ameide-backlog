...
### 3.2 Request Dev WorkRequest (agentic; Codex mocked; creates PR)
- **Input:** Accepted artifacts + task prompt + repo ref.
...
- **Next:** **Trigger** agent-work runner job; **Wait** 3.2a

### 3.2a Dev outcome recorded (PR created)
...
- **Next:** If success → **Trigger** 3.3; else **Loop** 3.2 (revise prompt / fix runner)
...
### 3.4 Automatically request cluster UI harness verification (stable URLs; gateway overlay)
...
- **Next:** If the manifest indicates no `edge_routable` services are affected → **Skip** 3.4 and **Trigger** Phase 4; else **Trigger** E2E runner job; **Wait** 3.4a
